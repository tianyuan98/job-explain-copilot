...
async def parse_resume(file: UploadFile = File(...)) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            print(f"[parse_resume] LLM 调用异常: {error_detail}", flush=True)
            return {"error": "llm_failed", "message": "AI 解析服务暂时不可用，请稍后重试或手动填写", "detail": error_detail}
